...
/// that the choice follows the user between the Seen, Watchlist and Favorites
/// screens rather than being set separately on each.
...
class _MyTabViewState extends State<MyTabView> {
  ...
  @override
  void didUpdateWidget(covariant MyTabView oldWidget) {
    super.didUpdateWidget(oldWidget);
    // The grid contents change when an item is added or removed elsewhere, and
    // any newly arrived item has no metadata yet.
    if (oldWidget.favItems.length != widget.favItems.length &&
        _sort.needsMetadata) {
      _loadMetadata();
    }
  }
  ...
  Widget build(BuildContext context) {
    final List<dynamic> items =
        sortMediaItems(widget.favItems, _sort, _metadata);
